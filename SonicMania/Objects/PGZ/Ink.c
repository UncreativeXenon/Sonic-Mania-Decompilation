            ...
            if (Player_CheckCollisionBox(player, self, &Ink->hitboxBottleBottom) == C_TOP) {
                if (!((1 << playerID) & self->inkedPlayers)) {
                    ...
                    switch (player->characterID) {
                        ...
                        // Fix:
                        // to fix this up to work as "intended", simply replace the "PLAYER_PALETTE_INDEX_SONIC_OLD"s with
                        // "PLAYER_PALETTE_INDEX_SONIC"
                        case ID_SONIC:
...
void Ink_StageLoad(void)
{
    ...
    for (p = 0; p < PLAYER_COUNT; ++p) Ink->playerColors[p] = 0;
}
