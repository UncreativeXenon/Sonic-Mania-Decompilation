...
void KleptoMobile_Hit(void)
{
    ...
    if (--self->health <= 0) {
...

        {
            ...
            {
                EntityKleptoMobile *arm1;
                EntityKleptoMobile *arm2; 
                ...
                arm1 = RSDK_GET_ENTITY(SceneInfo->entitySlot - 1, KleptoMobile);
                arm1->timer              = 0;
                arm1->state              = KleptoMobile_StateArm_Cutscene;

                arm2 = RSDK_GET_ENTITY(SceneInfo->entitySlot + 1, KleptoMobile);
                arm2->timer              = 0;
...
void KleptoMobile_HandleFrames(void)
{
    ...
    moveX = 0x1C00 * RSDK.Sin512(-self->rotation) + self->position.x;
    moveY = 0x1C00 * RSDK.Cos512(-self->rotation) + self->position.y;
...

    {
        ...
        foreach_active(PhantomKing, kingArm)
        {
            if (kingArm->type != PHANTOMKING_KING) {
                ...
                for (i = 0; i < 10; ++i) kingArm->armPositions[i] = kingPtr->position;
...
void KleptoMobile_State_SetupArena(void)
{
    ...
    hand = RSDK_GET_ENTITY(SceneInfo->entitySlot - 2, KleptoMobile);
    ...
    if (--self->bashArmDelay == 1) {
        ...
        if (self->canBashAttack) {
            if (self->bashArmDelay <= 0 && ++self->timer == 15) {
                int32 armSlot;
                EntityKleptoMobile *arm;
                self->timer = 0;
                RSDK.PlaySfx(KleptoMobile->sfxFlail, false, 0xFF);
                armSlot = self->bashArmID ? SceneInfo->entitySlot + 1 : SceneInfo->entitySlot - 1;

                arm = RSDK_GET_ENTITY(armSlot, KleptoMobile);
...
void KleptoMobile_HandleArmPositions(void)
{
    foreach_active(KleptoMobile, eggman)
    {
        if (eggman->type != KLEPTOMOBILE_EGGMAN) {
            ...
            for (i = 0; i < 10; ++i) eggman->armPositions[i] = parent->position;
...
void KleptoMobile_StateHand_Boss(void)
{
    ...
    parentY   = parent->position.y - 0x180000;
    ...
    if (self->direction == FLIP_X)
        ...
        x = ((parent->position.x + self->position.x) >> 1) - 0x80000;
    y = ((parentY + self->position.y) >> 1) - 0x300000;
...
void KleptoMobile_Draw_Hand(void)
{
    ...
    for (i = 0; i < 6; ++i) RSDK.DrawSprite(&self->orbAnimator, &self->armPositions[i], false);
...
void KleptoMobile_StateArm_BashAttack(void)
{
    ...
    if (self->timer >= 4) {
        ...
        distX = self->position.x - x;
        distY = self->position.y - y;
...
void KleptoMobile_StateArm_ChargeAttack(void)
{
    ...
    x2 = (x + self->position.x) >> 1;
    y2 = (y + self->position.y) >> 1;
