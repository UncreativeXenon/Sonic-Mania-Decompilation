...
static LeaderboardID leaderboardInfo[] = {
    {0,   1,   71,  "PlusGHZ1SonicMania",      "GHZ1_S_MANIA" },
...
void TimeAttackData_MigrateLegacySaves(void)
{
    if (globals->saveLoaded == STATUS_OK) {
        ...
        for (zone = ZONE_GHZ; zone <= ZONE_TMZ; ++zone) {
            int32 act;
            for (act = ACT_1; act <= ACT_2; ++act) {
                int32 charID;
                for (charID = CHAR_SONIC; charID <= CHAR_KNUX; ++charID) {
                    int32 rank;
...
int32 TimeAttackData_AddDBRow(uint8 zoneID, uint8 act, uint8 characterID, uint8 mode, int32 score)
{
    ...
    rowID    = API.AddUserDBRow(globals->taTableID);
    encore    = mode & 1;
    replayID = 0;
    ...
    uuid = API.GetUserDBRowUUID(globals->taTableID, rowID);
...
int32 TimeAttackData_GetScore(uint8 zoneID, uint8 act, uint8 characterID, bool32 encore, int32 rank)
{
    ...
    rowID = API.GetSortedUserDBRowID(globals->taTableID, rankID);
    if (rowID == -1)
        return 0;
...
int32 TimeAttackData_GetReplayID(uint8 zoneID, uint8 act, uint8 characterID, bool32 encore, int32 rank)
{
    ...
    row = API.GetSortedUserDBRowID(globals->taTableID, rankID);
    if (row != -1) {
        int32 replayID = 0;
...
{
    StatInfo stat;
    LeaderboardID *curLeaderboard;
    TimeAttackData_TrackTAClear(&stat, zoneID, act, characterID, isEncore, score);
    API.TryTrackStat(&stat);

    curLeaderboard = TimeAttackData_GetLeaderboardInfo(zoneID, act, characterID, isEncore);
