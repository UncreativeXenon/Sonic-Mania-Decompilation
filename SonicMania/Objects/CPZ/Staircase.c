...
void Staircase_Create(void *data)
{
    ...
    add = self->direction ? -0x200000 : 0x200000;

    for (i = 0; i < STAIRCASE_STAIR_COUNT; ++i) {
...
void Staircase_State_Idle(void)
{
    ...
    foreach_active(Player, player)
    {
        for (i = 0; i < STAIRCASE_STAIR_COUNT; i++) {
            ...
            side     = Player_CheckCollisionBox(player, self, &Staircase->blockHitbox);
            if (side == C_BOTTOM) {
...
void Staircase_State_MoveBlocks(void)
{
    ...
    for (i = 0; i < STAIRCASE_STAIR_COUNT; ++i) self->blockPos[i].y += (i + 1) * add;
...
void Staircase_EditorDraw(void)
{
    ...
    if (showGizmos()) {
        ...
        addY = self->type ? -0x200000 : 0x200000;
        for (i = 0; i < STAIRCASE_STAIR_COUNT; ++i) self->blockPos[i].y += (i + 1) * addY;
